...
#
# Tujuan
# Pertama, kita akan melanjutkan proyek analisis data Dicoding Collection dengan menjalankan proses membuat dashboard
# sederhana. Proses ini memiliki tujuan antara lain:
#
...
#
# Persiapan.
# 1. Menyiapkan DataFrame yang akan digunakan.
# 2. Membuat komponen filter pada dashboard.
# 3. Melengkapi dashboard dengan berbagai visualisasi data.
...
all_df = pd.read_csv(Path(__file__).parents[1] / "learnings/all_data.csv")
...
sns.barplot(y="recency", x="customer_id", data=rfm_df.sort_values(by="recency", ascending=True).head(5), palette=colors,
            ax=ax[0], hue="customer_id", legend=False)
ax[0].set_ylabel(None)
...
sns.barplot(y="frequency", x="customer_id", data=rfm_df.sort_values(by="frequency", ascending=False).head(5),
            palette=colors, ax=ax[1], hue="customer_id", legend=False)
ax[1].set_ylabel(None)
...
sns.barplot(y="monetary", x="customer_id", data=rfm_df.sort_values(by="monetary", ascending=False).head(5),
            palette=colors, ax=ax[2], hue="customer_id", legend=False)
